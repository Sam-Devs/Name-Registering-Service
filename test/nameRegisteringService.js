...
contract("NameRegisteringService", ([owner, wallet, anotherAccount]) => {
  ...
  beforeEach(async () => {
    contractInstance = await NameRegisteringService.new();
  });
  ...
  it("register Should raise LogReceipt event when successfully registering a domain", async () => {
    ...
    const result = await promiEvent;
    const now = web3.eth.getBlock(
      initialTransaction.receipt.blockNumber
    ).timestamp;
